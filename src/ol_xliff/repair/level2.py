...
def _shield_to_xml(text: str) -> tuple[str, dict[str, str]]:
    ...
    def _replacer(m: re.Match) -> str:
        shield = m.group(0)
        inner = m.group(1)
        # Sanitise the inner name so it's a valid XML tag name.
        safe = inner.replace("/", "_slash_").replace("-", "_dash_")
        xml_tag = f"<ol_{safe}>"
        reverse[xml_tag] = shield
        # Also handle the closing variant if this is an opening tag.
...
def level2_span_align(
    ...
    try:
        # SpanProjector expects XML-like tags; convert shields to XML first.
